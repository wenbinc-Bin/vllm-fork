    ...
    def forward(
        ...
        for layer_num, blk in enumerate(self.blocks):
            hidden_states = blk(hidden_states,
                                ...
                                max_seqlen=max_seqlen,
                                seqlens=seqlens)
            if deepstack_visual_indexes is not None and \
               layer_num in deepstack_visual_indexes:
        ...
        if deepstack_visual_indexes is not None:
            ...
            # we cat the original visual features and
            # deepstack features along the feature dim
        ...
        for layer_idx, layer in enumerate(
            ...
            if input_embeds_multiscale is not None and \
                        layer_idx in range(
                            ...
                            (self.deepstack_multiscale_layer_start +
                             len(input_embeds_multiscale))):
                input_embeds_multiscale_this = input_embeds_multiscale[
    ...
    def _get_prompt_updates(
        ...
        def get_replacement_qwen2_vision(item_idx: int, modality: str):
            ...
            token_id = (image_token_id
                        if modality == "image" else video_token_id)
...
class Qwen3OmniMoeConditionalGenerationMixin(
    ...
    def _process_image_input(
        ...
        if is_hpu:
            assert isinstance(self.visual, Qwen3_VisionTransformerStaticShape)
...
class Qwen3OmniMoeThinkerForConditionalGeneration(
    ...
    def _get_deepstack_input_embeds(self) -> IntermediateTensors:
        ...
        return self.deepstack_input_embeds
            ...
            if self.visual.deepstack_visual_indexes is not None:
                ...
                if len(multimodal_embeddings_multiscale) > 0:
                    ...
                    input_embeds_multiscale = input_embeds_multiscale.view(
                        ...
                        multiscale_len, visual_dim).permute(2, 0, 1,
                                                            3).contiguous()
    ...
    def get_input_embeddings_v0(
        ...
        if use_deepstack:
            ...
            for embeddings, modality in multimodal_embeddings:
                if modality in ["image", "video"]:
                    input_embeds_multiscale = torch.zeros_like(
        ...
        if use_deepstack and input_embeds_multiscale is not None:
            input_embeds_multiscale = input_embeds_multiscale.view(
                ...
                visual_dim).permute(2, 0, 1, 3).contiguous()
            self._set_deepstack_input_embeds(input_embeds_multiscale)
            return inputs_embeds
