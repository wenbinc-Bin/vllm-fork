...
class MRotaryEmbedding(RotaryEmbedding):
        ...
        if input_ids is not None and (image_grid_thw is not None
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                remain_images, remain_videos, remain_audios = \
                    image_nums, video_nums, audio_nums
                ...
                for _ in range(multimodal_nums):
                    ...
                    if min_ed == ed_audio_start:
                        ...
                        video_idx += 1
                        remain_videos -= 1
                    elif min_ed == ed_vision_start and \
                         ed_vision_start + 1 == ed_audio_start and \
                         use_audio_in_video:
                        text_len = min_ed - st
                        if text_len != 0:
                            st_idx = llm_pos_ids_list[-1].long().max(
                        ...
                        llm_pos_ids_list.append(
                            torch.arange(eos_len).view(1, -1).expand(3, -1) +
                            st_idx)
                        st += text_len + bos_len * 2 + \
                              audio_len + video_len + eos_len * 2
                        audio_idx += 1
                        video_idx += 1
                        remain_videos -= 1
                        remain_audios -= 1
    ...
    def _omni_get_input_positions_tensor(
        ...
        if "qwen3_omni" in model_type:
            llm_positions, mrope_position_delta = \
              cls._omni3_get_input_positions_tensor(
