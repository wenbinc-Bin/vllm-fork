...
if TYPE_CHECKING:
    ...
    VLLM_XPU_DETERMINISTIC: bool = False
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Enable torch.use_deterministic_algorithms on XPU workers. Some XPU
    # oneDNN/oneMKL matmul kernels use non-deterministic (split-K/atomic)
    # reductions, which makes greedy decode non-reproducible run-to-run.
    # Enabling this trades some performance for bit-reproducible results.
    "VLLM_XPU_DETERMINISTIC": lambda: bool(
        int(os.getenv("VLLM_XPU_DETERMINISTIC", "0"))
    ),
